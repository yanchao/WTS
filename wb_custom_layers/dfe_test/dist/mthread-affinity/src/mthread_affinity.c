...
void printThreadInfo(void * i) {
    ...
    int new_core_id ,thread_id;
    int old_core_id = sched_getcpu();
    int ret;

    new_core_id = thread_id = (int)i;
    char threadStr[16] = "Thread ";
    sprintf (&(threadStr[strlen(threadStr)]), "%ld", old_core_id);
    printInfo((const char *) threadStr);
    prctl(PR_SET_NAME, "yzhang0 Task", 0, 0, 0);

    printf ("Thread %d is awake coreid = %d .\n", thread_id, old_core_id);
    /* keep thread going forever */
    while (1) {
        if (new_core_id != old_core_id) {
            printf ("Thread %ld is transfering from core %d to core %d.\n", thread_id, old_core_id, new_core_id);
            /* set CPU Affinity */
            CPU_ZERO(&mask);
            CPU_SET(new_core_id,&mask);
            ret = pthread_setaffinity_np(self,len,&mask);
            if (0 != ret) {
                printf("set affinity to core %d failed ! ret is %d !! \n", new_core_id, ret);
                exit(-1);
            }
            new_core_id = sched_getcpu();
            printf ("Thread %ld is transfered from core %d to core %d.\n", thread_id, old_core_id, new_core_id);
            old_core_id = new_core_id;
        }
        new_core_id = sched_getcpu();
        printf ("Thread %ld is awake coreid = %d (pid %d).\n", thread_id, new_core_id, getpid());
        sleep(2);
        sharedCode(); 
    }
...
int main(int argc, char *argv[]) {
    ...
    retval = pthread_create(&threads[nr_threads], NULL, (VOIDFUNCPTR) alone, 0);
    nr_threads++; 
