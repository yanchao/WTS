...
void   printInfo(const char * header) {
    ...
    printf("%s :\tProcessID =%8d ParentID = %8d\n",
            header, (int) getpid(),  getppid());
}
...
void printProcessInfo(long i) {
	...
	pid_t self = getpid();
    int new_core_id ,process_id;
    int old_core_id = sched_getcpu();
    int ret;

    new_core_id = process_id = (int)i;
    prctl(PR_SET_NAME, "yzhang0 process", 0, 0, 0);
    printf ("Process %d is awake on coreid = %d ,pid = %d .\n", process_id, old_core_id,(int) self);
    /* keep process going forever */
    while (1) {
        if (new_core_id != old_core_id) {
            printf ("Process %ld is transfering from core %d to core %d.\n", process_id, old_core_id, new_core_id);
            /* set CPU Affinity */
            CPU_ZERO(&mask);
            CPU_SET(new_core_id,&mask);
            ret = sched_setaffinity(self,len,&mask);
            if (0 != ret) {
                printf("set affinity to core %d failed ! ret is %d !! \n", new_core_id, ret);
                exit(-1);
            }
            new_core_id = sched_getcpu();
            printf ("Process %ld is transfered from core %d to core %d.\n", process_id, old_core_id, new_core_id);
            old_core_id = new_core_id;
        }
        new_core_id = sched_getcpu();
        printf ("Process %ld is awake coreid = %d (pid %d).\n", process_id, new_core_id, getpid());
        sleep(2);
        sharedCode(); 
    }
...
int main(int argc, char *argv[]) {
    ...
    /*  get system logic cpu number */
    nr_process = sysconf(_SC_NPROCESSORS_ONLN);
    /********Complicated structure test*******/
    ...
    retval = fork();
    if (0 > retval) {
        printf ("\n fork child process alone failed  !" );
        exit(1);
    } else if (0 == retval) {
        alone();
        exit(1);
